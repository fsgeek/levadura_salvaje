"""Run the currency lens (Jev) over the 2025 fossil-candidate sections.

Predictions C1-C8: predictions/2026-09-24-currency-lens-claude.md. The lens
(lenses/currency.py) was written by a separate instance from the intent
...
as it completes. Section label = strongest chunk label, current > historical
> no_rules.

Usage: uv run python scripts/measure_currency_lens.py [--pilot N]
"""
...
INSTRUMENT = {
    "name": "levadura_salvaje.lenses.currency",
    "version": currency.LENS_VERSION,
    "model": currency.MODEL,
    "lens_sha256": hashlib.sha256(currency.LENS_TEXT.encode()).hexdigest(),
    "method": (
        ...
        "strongest chunk label (current > historical > no_rules)"
    ),
    "lens_written_by": "a separate instance, from the intent section of the predictions only",
    "predictions": "predictions/2026-09-24-currency-lens-claude.md",
}
...
def ask(client, s, b, k, n, piece):
    ...
    r = currency.classify(state, client)
...
def run(pilot: int | None = None) -> None:
    edition = EDITION
    partial = Path(f"results/currency-jev-v{currency.LENS_VERSION}-{edition}.partial.jsonl")
    ...
    cit = citation_rows()
    final = Path(f"results/currency-jev-v{currency.LENS_VERSION}-{edition}.jsonl")
    rows = []
    ...
    heavy = [r for r in rows if r["broken"] * 2 >= r["citations"]]
    light = [r for r in rows if r["broken"] * 2 < r["citations"]]
    sub_only = [r for r in rows if set(r["broken_kinds"]) == {"absent-subdivision"}]
    sec_rep = [r for r in rows if "repealed" in r["broken_kinds"] or "absent-section" in r["broken_kinds"]]
    ...
    rec = append({
        ...
        "quantity": "currency_lens_section_labels",
        "value": {
            "sections": len(rows),
            "labels": {k: labels.get(k, 0) for k in STRENGTH},
            "C4_historical_heavy": share(heavy, "historical"),
            "C4_historical_light": share(light, "historical"),
            "C5_amt_fossils_historical": share(amt, "historical"),
            "C6_current_subdivision_only": share(sub_only, "current"),
            "C6_current_section_level": share(sec_rep, "current"),
